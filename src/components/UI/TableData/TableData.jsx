...
const TableData = ({ columns, rows, rowAction, isColored, select, filterLabel }) => {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      {select
        ? <>
        <Typography sx={{ m: 1, fontFamily: "'Overpass',sans-serif" }} variant="h7" color="text.regular">
          {filterLabel}
        </Typography>
        {select}
      </>
          ...
          <TableBody>
            {rows
              ...
              <TableRow key={index} sx={{
                ...
                {row.map((cell, index) => (
                  <TableCell key={index} sx={{ ...TableStyles.tableBodyCell, ...((typeof (cell) === "number") && isColored && (cell > 0 ? { color: "trade.primary" } : parseFloat(cell) === 0 ? { color: "text.regular" } : { color: "error.primary" })) }}>
                    {cell}
                  </TableCell>
                ))}
